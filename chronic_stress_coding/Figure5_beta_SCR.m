%% SCR beta-coefficient analysis: Chronic stress study
% Publication/repository version
%
% Purpose
%   Compare participant-level skin-conductance-response (SCR) GLM
%   coefficients between Healthy controls and participants with Chronic stress.
%
% Computational models analysed separately:
%   1. Surprise
...
%       Predictor = Accuracy coefficient, Model coefficient
%
% Primary analysis
...
%   C. Accuracy coefficient vs Model coefficient within each group and
%      controllability condition
%
% Cleaning
%   - S1 and S2 are paired within participant.
%   - Healthy controls are paired by Subject/SubjectID when available;
%     otherwise row order is used with a warning.
%   - Optional >3 SD exclusion is also applied at the participant-row level.
%
% Data mapping
%   Healthy files:
%       zCorr = prediction-accuracy coefficient
%       zUnc  = computational-model coefficient
%
%   Chronic-stress files:
%       Beta_Accuracy = prediction-accuracy coefficient
%       Beta_U        = computational-model coefficient
%
...
dataDir = fullfile(scriptDir,'data','SCR_Betas');
...
outDir   = fullfile(scriptDir,'results','SCR_Betas');
...
healthyS1Files = {
    fullfile(healthyS1Dir,'healthy_zUnc_zCorr_surprise_s1.xlsx'), ...
    fullfile(healthyS1Dir,'healthy_zUnc_zCorr_estimation_s1.xlsx'), ...
    fullfile(healthyS1Dir,'healthy_zUnc_zCorr_volatility_s1.xlsx')
    };

healthyS2Files = {
    fullfile(healthyS2Dir,'healthy_zUnc_zCorr_surprise_s2.xlsx'), ...
    fullfile(healthyS2Dir,'healthy_zUnc_zCorr_estimation_s2.xlsx'), ...
    fullfile(healthyS2Dir,'healthy_zUnc_zCorr_volatility_s2.xlsx')
    };

chronicS1Files = {
    fullfile(chronicS1Dir,'SCR_Betas_Surprise_CS_S1.xlsx'), ...
    fullfile(chronicS1Dir,'SCR_Betas_Belief_Uncertainty_CS_S1.xlsx'), ...
    fullfile(chronicS1Dir,'SCR_Betas_Volatility_CS_S1.xlsx')
    };

chronicS2Files = {
    fullfile(chronicS2Dir,'SCR_Betas_Surprise_CS_S2.xlsx'), ...
    fullfile(chronicS2Dir,'SCR_Betas_Belief_Uncertainty_CS_S2.xlsx'), ...
    fullfile(chronicS2Dir,'SCR_Betas_Volatility_CS_S2.xlsx')
    };
...
%% Required columns
healthyRequiredColumns = {'zUnc','zCorr'};
chronicRequiredColumns = {'SubjectID','Beta_U','Beta_Accuracy'};

%% Cleaning settings
apply3SD = true;
sdThreshold = 3;

% Nearest-value matching tolerance for the recorded manual exclusions.
manualTolerance = 0.02;

%% Recorded manual exclusions
% Internal matrix order:
%   1 = S1 Accuracy
%   2 = S1 Model
%   3 = S2 Accuracy
%   4 = S2 Model
%
% A successful match removes the entire participant row for that model.
...
% Fixed y-axis retained from the original figure.
useFixedYLim = true;
fixedYLim = [-0.78 0.38];
fixedYTicks = [-0.78 -0.39 0.00 0.38];
fixedYTickFormat = '%.2f';
...
for m = 1:3
    ...
    fprintf('\n============================================================\n');
    fprintf('SCR beta model %d/3: %s\n',m,modelName);
    fprintf('============================================================\n');

    %% Read source tables
    H1 = read_healthy_beta_table(healthyS1Files{m},healthyRequiredColumns);
    H2 = read_healthy_beta_table(healthyS2Files{m},healthyRequiredColumns);

    C1 = read_chronic_beta_table(chronicS1Files{m},chronicRequiredColumns);
    C2 = read_chronic_beta_table(chronicS2Files{m},chronicRequiredColumns);

    %% Pair S1 and S2
    [healthyIDs,MH0] = pair_healthy_sessions(H1,H2);
    [chronicIDs,MC0] = pair_chronic_sessions(C1,C2);
    ...
    %% Recorded manual participant-level exclusions
    ...
    %% Post-hoc C: Accuracy coefficient vs Model coefficient
    GroupC = [ ...
        "Healthy";"Healthy"; ...
        "Chronic";"Chronic"];
    ...
    for k = 1:4
        ...
        meanH = mean(yH);
        semH = std(yH)/sqrt(numel(yH));

        errorbar(ax,xH(colIdx),meanH,semH,'k', ...
            'LineWidth',lineMean,'CapSize',0);

        scatter(ax,xH(colIdx),meanH,meanSize,'k','filled');
        ...
        meanC = mean(yC);
        semC = std(yC)/sqrt(numel(yC));

        errorbar(ax,xC(colIdx),meanC,semC,'k', ...
            'LineWidth',lineMean,'CapSize',0);

        scatter(ax,xC(colIdx),meanC,meanSize,'k','filled');
    end
    ...
    figureBase = fullfile(outDir, ...
        sprintf('SCR_Betas_Healthy_vs_Chronic_%s',safeModel));
    ...
    statsExcel = fullfile(statsDir, ...
        sprintf('SCR_Betas_%s_stats.xlsx',safeModel));

    statsText = fullfile(statsDir, ...
        sprintf('SCR_Betas_%s_stats.txt',safeModel));

    statsMat = fullfile(statsDir, ...
        sprintf('SCR_Betas_%s_stats.mat',safeModel));
    ...
    analysisInfo = table( ...
        ["Computational model"; ...
         ...
         "Healthy accuracy coefficient"; ...
         "Healthy model coefficient"; ...
         "Chronic accuracy coefficient"; ...
         "Chronic model coefficient"; ...
         "Manual exclusions"; ...
         "3-SD removal"; ...
         "SD threshold"; ...
         "Healthy N"; ...
        ...
        [string(modelName); ...
         ...
         "Predictor: Accuracy coefficient vs Model coefficient"; ...
         "zCorr"; ...
         "zUnc"; ...
         "Beta_Accuracy"; ...
         "Beta_U"; ...
    ...
    fprintf(fid,'SCR BETA-COEFFICIENT ANALYSIS: CHRONIC STRESS STUDY\n');
    fprintf(fid,'====================================================\n\n');

    fprintf(fid,'Computational model: %s\n\n',modelName);

    fprintf(fid,'Healthy mapping:\n');
    fprintf(fid,'  zCorr = prediction-accuracy coefficient\n');
    fprintf(fid,'  zUnc  = computational-model coefficient\n\n');

    fprintf(fid,'Chronic-stress mapping:\n');
    fprintf(fid,'  Beta_Accuracy = prediction-accuracy coefficient\n');
    fprintf(fid,'  Beta_U        = computational-model coefficient\n\n');
...
fprintf('\nAll SCR beta-coefficient analyses completed.\n');
fprintf('Figures: %s\n',outDir);
...
function T = read_healthy_beta_table(filePath,requiredColumns)
    ...
    if ~isempty(missingColumns)
        error('Healthy file %s is missing required columns: %s', ...
            filePath,strjoin(missingColumns,', '));
    end

    T.zUnc = double(T.zUnc);
    T.zCorr = double(T.zCorr);
end


function T = read_chronic_beta_table(filePath,requiredColumns)
    ...
    if ~isempty(missingColumns)
        error('Chronic-stress file %s is missing required columns: %s', ...
            filePath,strjoin(missingColumns,', '));
    end

    T.Beta_U = double(T.Beta_U);
    T.Beta_Accuracy = double(T.Beta_Accuracy);
end


function [baseIDs,M] = pair_healthy_sessions(T1,T2)

    idColumn1 = find_subject_column(T1);
    idColumn2 = find_subject_column(T2);

    if ~isempty(idColumn1) && ~isempty(idColumn2)

        id1 = normalize_subject_id(string(T1.(idColumn1)));
        id2 = normalize_subject_id(string(T2.(idColumn2)));

        [baseIDs,i1,i2] = intersect(id1,id2,'stable');

        if isempty(baseIDs)
            error('No S1/S2 participant matches found for Healthy controls.');
        end

    else
        warning(['Healthy Subject/SubjectID column not found in both sessions. ' ...
                 'Pairing S1 and S2 by row order. Verify that participant ' ...
                 'order is identical across the two healthy files.']);

        n = min(height(T1),height(T2));

        i1 = (1:n)';
        i2 = (1:n)';
        baseIDs = "H" + compose("%02d",(1:n)');
    end

    % Internal order:
    % S1 Accuracy, S1 Model, S2 Accuracy, S2 Model
    M = [ ...
        T1.zCorr(i1), ...
        T1.zUnc(i1), ...
        T2.zCorr(i2), ...
...
function [baseIDs,M] = pair_chronic_sessions(T1,T2)

    id1 = normalize_subject_id(string(T1.SubjectID));
    id2 = normalize_subject_id(string(T2.SubjectID));

    [baseIDs,i1,i2] = intersect(id1,id2,'stable');

    if isempty(baseIDs)
        error('No S1/S2 participant matches found for Chronic stress.');
    end
...
function name = find_subject_column(T)
    ...
    candidates = {'Subject','SubjectID','Participant','ParticipantID'};
    name = '';
...
function id = normalize_subject_id(id)
    ...
    % Remove common session/cleaning suffixes while preserving the base ID.
